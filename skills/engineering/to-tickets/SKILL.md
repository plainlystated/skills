...
Publish the approved tickets as GitHub issues, one per ticket, in dependency order (blockers first) so each ticket's blocking edges can reference real issue numbers. Use GitHub's native blocking / sub-issue relationship where available; otherwise set each ticket's "Blocked by" to the blocking issues.
...
<exactly one of: HITL or AFK — write only the chosen value, e.g. `HITL` on its own line>
